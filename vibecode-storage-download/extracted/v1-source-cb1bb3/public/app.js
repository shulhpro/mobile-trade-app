class MobileTradeApp {
  ...
  async init() {
    this.bindEvents();
    if (!this.authToken) { this.showToast('Требуется авторизация', 'error'); return; }
    try {
  ...
  async apiCall(endpoint, options = {}) {
    ...
    const config = { headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + this.authToken }, ...options };
  ...
  renderCompanies(companies) {
    ...
    if (companies.length === 0) { container.innerHTML = '<div class="empty-state"><div class="empty-state-icon">🏢</div><p>Компании не найдены</p></div>'; return; }
  ...
  getGPS() {
    if (!navigator.geolocation) { document.getElementById('gpsCoords').textContent = 'GPS не поддерживается'; return; }
    document.getElementById('gpsCoords').textContent = 'Определение...';
    navigator.geolocation.getCurrentPosition(
      (position) => { this.gpsCoords = { lat: position.coords.latitude, lng: position.coords.longitude }; document.getElementById('gpsCoords').textContent = this.gpsCoords.lat.toFixed(6) + ', ' + this.gpsCoords.lng.toFixed(6); },
      (error) => { document.getElementById('gpsCoords').textContent = 'Не удалось определить'; console.error('GPS error:', error); },
  ...
  handlePhotos(event) {
    ...
    toAdd.forEach(file => {
      ...
      reader.onload = (e) => { this.photos.push({ name: file.name, data: e.target.result, file: file }); this.renderPhotos(); };
  ...
  removePhoto(index) { this.photos.splice(index, 1); this.renderPhotos(); }

  async openProducts() { this.showScreen('productsScreen'); await this.loadSections(); }

  async loadSections() {
    try { const data = await this.apiCall('/api/catalog/sections'); this.sections = data.result?.sections || []; this.renderSections(); }
    catch (error) { console.error('Error loading sections:', error); this.showToast('Ошибка загрузки разделов', 'error'); }
  }
  ...
  renderSections() {
    ...
    if (this.sections.length === 0) { container.innerHTML = '<div class="empty-state"><div class="empty-state-icon">📁</div><p>Разделы не найдены</p></div>'; return; }
  ...
  async selectSection(sectionId) {
    try {
      ...
      this.renderProducts();
    } catch (error) { console.error('Error loading products:', error); this.showToast('Ошибка загрузки товаров', 'error'); }
  }

  showSections() { document.getElementById('sectionsView').style.display = 'block'; document.getElementById('productsView').style.display = 'none'; }

  renderProducts() {
    const container = document.getElementById('productsList');
    if (this.products.length === 0) { container.innerHTML = '<div class="empty-state"><div class="empty-state-icon">📦</div><p>Товары не найдены</p></div>'; return; }
  ...
  updateQuantity(productId, delta) {
    ...
    if (existingItem) {
      ...
      if (existingItem.quantity <= 0) { this.orderItems = this.orderItems.filter(item => item.id !== productId); }
      else { existingItem.total = existingItem.quantity * existingItem.price; }
    } else if (delta > 0) { this.orderItems.push({ id: product.id, name: product.name, article: product.xmlId || '', price: parseFloat(product.price), quantity: 1, total: parseFloat(product.price) }); }
    this.renderProducts();
  }
  ...
  renderCart() {
    ...
    if (this.orderItems.length === 0) { container.innerHTML = '<p style="text-align:center;color:#666">Корзина пуста</p>'; return; }
  ...
  async saveVisit(complete = false) {
    ...
    try {
      ...
      for (const photo of this.photos) {
        try { const fileData = await this.uploadFile(photo.file, photo.name); if (fileData.result?.ID) uploadedFiles.push(fileData.result.ID); }
      ...
      const taskData = { taskId: this.currentTask, title: 'Визит к ' + this.currentCompany.TITLE, description: description, companyId: this.currentCompany.ID, responsibleId: this.currentUser.ID, projectId: projectId, auditors: auditors, files: uploadedFiles };
  ...
  buildDescription() {
    ...
    if (this.orderItems.length > 0) {
      ...
      let total = 0;
      this.orderItems.forEach(item => { parts.push((item.article || '-') + ' | ' + item.name + ' | ' + item.quantity + ' | ' + item.price + ' ₽ | ' + item.total + ' ₽'); total += item.total; });
      parts.push('—'.repeat(50));
  ...
  async createOrderSubtask(parentId, projectId, auditors) {
    try {
      ...
      this.orderItems.forEach(item => { description += (item.article || '-') + ' | ' + item.name + ' | ' + item.quantity + ' | ' + item.price + ' ₽ | ' + item.total + ' ₽' + String.fromCharCode(10); });
      description += '—'.repeat(50) + String.fromCharCode(10);
      ...
      const excelData = await this.apiCall('/api/excel/order', { method: 'POST', body: { clientName: clientName, date: new Date().toLocaleDateString('ru-RU'), items: this.orderItems, total: total } });
      ...
      const subtaskData = { parentId: parentId, title: 'Заказ (' + clientName + ')', description: description, responsibleId: this.currentUser.ID, projectId: projectId, auditors: auditors };
      if (excelFileId) subtaskData.files = [excelFileId];
      await this.apiCall('/api/tasks/subtask', { method: 'POST', body: subtaskData });
    } catch (error) { console.error('Error creating order subtask:', error); }
  }

  async uploadFile(file, fileName) {
    const formData = new FormData();
    formData.append('file', file, fileName);
    const response = await fetch(this.apiBase + '/api/upload', { method: 'POST', headers: { 'Authorization': 'Bearer ' + this.authToken }, body: formData });
  ...
  async loadDashboard() {
    try {
      ...
      if (dateFrom || dateTo) { const params = new URLSearchParams(); if (dateFrom) params.append('dateFrom', dateFrom); if (dateTo) params.append('dateTo', dateTo); url += '?' + params.toString(); }
      ...
      let userNames = {};
      if (employeeIds.length > 0) { const usersData = await this.apiCall('/api/users?ids=' + employeeIds.join(',')); (usersData.result || []).forEach(user => { userNames[user.ID] = user.NAME + ' ' + user.LAST_NAME; }); }
    ...
    } catch (error) { console.error('Error loading dashboard:', error); this.showToast('Ошибка загрузки аналитики', 'error'); }
  }
  ...
  showLoading(text) { document.getElementById('loadingText').textContent = text; document.getElementById('loadingModal').classList.add('active'); }
  hideLoading() { document.getElementById('loadingModal').classList.remove('active'); }
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => { navigator.serviceWorker.register('/sw.js').then(reg => console.log('SW registered')).catch(err => console.log('SW error:', err)); });
}
