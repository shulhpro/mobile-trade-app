п»ї// Mobile Trade App - Frontend Logic
  ...
  try {
    ...
    if (data.user) {
      ...
      document.getElementById('userInfo').textContent = 
        (user.name || '') + ' ' + (user.lastName || '').trim() || user.email || 'РџРѕР»СЊР·РѕРІР°С‚РµР»СЊ';
    }
...
function renderProjectSelector(workgroups) {
  ...
  select.innerHTML = '<option value="">-- Р’С‹Р±РµСЂРёС‚Рµ РїСЂРѕРµРєС‚ --</option>';
...
function renderCompanies(companiesList) {
  ...
  container.innerHTML = companiesList.map(company => {
    const address = company.address || company.ADDRESS || 'РђРґСЂРµСЃ РЅРµ СѓРєР°Р·Р°РЅ';
...
async function selectCompany(companyId) {
  ...
  const address = currentCompany.address || currentCompany.ADDRESS || 'РђРґСЂРµСЃ РЅРµ СѓРєР°Р·Р°РЅ';
  const phone = currentCompany.phone || (currentCompany.fm && currentCompany.fm.find(f => f.typeId === 'PHONE')?.value) || 'РќРµ СѓРєР°Р·Р°РЅ';
  const email = currentCompany.email || (currentCompany.fm && currentCompany.fm.find(f => f.typeId === 'EMAIL')?.value) || 'РќРµ СѓРєР°Р·Р°РЅ';
  
  let taskInfo = '';
  if (currentTask) {
    taskInfo = '<div class="task-info" style="margin-top: 10px; padding: 10px; background: #e3f2fd; border-radius: 8px;">' +
      '<div style="font-weight: 600; color: #1976d2;">рџ“ќ Р•СЃС‚СЊ РѕС‚РєСЂС‹С‚Р°СЏ Р·Р°РґР°С‡Р°</div>' +
      '<div style="font-size: 12px; color: #666; margin-top: 4px;">РќРѕРІС‹Рµ РґР°РЅРЅС‹Рµ Р±СѓРґСѓС‚ РґРѕР±Р°РІР»РµРЅС‹ РІ СЃСѓС‰РµСЃС‚РІСѓСЋС‰СѓСЋ Р·Р°РґР°С‡Сѓ</div>' +
      '</div>';
  }
  
  card.innerHTML = '<h2>' + escapeHtml(currentCompany.title || currentCompany.TITLE) + '</h2>' +
    '<div class="detail-row"><span class="detail-label">рџ“Ќ РђРґСЂРµСЃ:</span><span>' + escapeHtml(address) + '</span></div>' +
    '<div class="detail-row"><span class="detail-label">рџ“ћ РўРµР»РµС„РѕРЅ:</span><span>' + escapeHtml(phone) + '</span></div>' +
    '<div class="detail-row"><span class="detail-label">вњ‰пёЏ Email:</span><span>' + escapeHtml(email) + '</span></div>' +
    taskInfo;
...
function getLocation() {
  ...
  navigator.geolocation.getCurrentPosition(
    (position) => {
      ...
      display.innerHTML = 'вњ… РњРµСЃС‚РѕРїРѕР»РѕР¶РµРЅРёРµ РѕРїСЂРµРґРµР»РµРЅРѕ<br>РЁРёСЂРѕС‚Р°: ' + currentLocation.latitude.toFixed(6) + '<br>Р”РѕР»РіРѕС‚Р°: ' + currentLocation.longitude.toFixed(6);
...
async function loadSections() {
  try {
    ...
    document.getElementById('sectionsList').innerHTML = 
      '<div class="loading">РћС€РёР±РєР° Р·Р°РіСЂСѓР·РєРё СЂР°Р·РґРµР»РѕРІ</div>';
  }
}
...
function renderSections() {
  ...
  if (sections.length === 0) {
    container.innerHTML = '<div class="loading">Р Р°Р·РґРµР»С‹ РЅРµ РЅР°Р№РґРµРЅС‹</div>';
    return;
  }
  
  container.innerHTML = sections.map(section => {
    return '<div class="section-item" onclick="loadProductsBySection(' + section.id + ')">' +
      '<div class="section-icon">рџ“Ѓ</div>' +
      '<div class="section-name">' + escapeHtml(section.name) + '</div>' +
      '<div class="section-arrow">в†’</div>' +
      '</div>';
  }).join('');
...
function renderProducts() {
  ...
  container.innerHTML = products.map(product => {
    ...
    return '<div class="product-item">' +
      ...
      '<button type="button" class="qty-btn" onclick="updateQty(' + productId + ', -1, ' + price + ')">в€’</button>' +
...
function updateQty(productId, delta, price) {
  ...
  const productName = product ? (product.name || product.NAME) : ('РўРѕРІР°СЂ ' + productId);
...
async function submitVisit(closeVisit) {
  ...
  formData.append('subject', 'Р’РёР·РёС‚ Рє ' + (currentCompany.title || currentCompany.TITLE));
  ...
  if (Object.keys(orderItems).length > 0) {
    const items = Object.entries(orderItems).map(([productId, item]) => {
      const qty = typeof item === 'object' ? item.quantity : item;
      const name = typeof item === 'object' ? item.name : ('РўРѕРІР°СЂ ' + productId);
      const price = typeof item === 'object' ? item.price : 0;
      return {
        name: name,
        quantity: qty,
  ...
  try {
    ...
    if (data.success) {
      if (closeVisit) {
        showSuccess('Р’РёР·РёС‚ Р·Р°РІРµСЂС€РµРЅ!', 'Р—Р°РґР°С‡Р° Р·Р°РєСЂС‹С‚Р°. Р’СЃРµ РґР°РЅРЅС‹Рµ СЃРѕС…СЂР°РЅРµРЅС‹.');
      } else {
        showSuccess('Р”Р°РЅРЅС‹Рµ СЃРѕС…СЂР°РЅРµРЅС‹!', 'РџСЂРѕРґРѕР»Р¶Р°Р№С‚Рµ РІРёР·РёС‚ РёР»Рё Р·Р°РєСЂРѕР№С‚Рµ РµРіРѕ');
      }
    } else {
...
function showLoadingOverlay(isClosing) {
  ...
  text.textContent = isClosing ? 'Р—Р°РІРµСЂС€РµРЅРёРµ РІРёР·РёС‚Р°...' : 'РЎРѕС…СЂР°РЅРµРЅРёРµ РґР°РЅРЅС‹С…...';
...
async function loadDashboard() {
  ...
  try {
    showToast('Р—Р°РіСЂСѓР·РєР° РґР°С€Р±РѕСЂРґР°...');
    ...
    if (data.success) {
      ...
      showToast('Р”Р°С€Р±РѕСЂРґ РѕР±РЅРѕРІР»РµРЅ');
    } else {
      showToast('РћС€РёР±РєР° Р·Р°РіСЂСѓР·РєРё РґР°С€Р±РѕСЂРґР°');
    }
  } catch (error) {
...
function formatMoney(amount) {
  if (!amount || amount === 0) return '0 в‚Ѕ';
  return amount.toLocaleString('ru-RU', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' в‚Ѕ';
}
