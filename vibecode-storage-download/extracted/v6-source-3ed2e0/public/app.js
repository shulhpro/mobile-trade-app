class MobileTradeApp {
  ...
  initBitrix() {
    if (typeof BX24 !== 'undefined') {
      ...
      BX24.init(() => {
        console.log('Bitrix24 initialized successfully');
        this.bitrixReady = true;
        this.authToken = BX24.getAuth();
        this.currentUser = {
          ID: BX24.getUserId(),
          NAME: 'User'
        };
      ...
      console.log('BX24 not available, using direct mode');
      this.init();
    }
  }
  ...
  async init() {
    ...
    try {
      if (this.bitrixReady) {
        ...
        // Show auth screen or try without auth
        this.showScreen('clientScreen');
        this.showToast('Режим без авторизации', 'warning');
      }
    } catch (error) {
  ...
  async apiCall(endpoint, options = {}) {
    ...
    if (this.bitrixReady && this.authToken) {
  ...
  async loadUserData() {
    if (this.bitrixReady && typeof BX24 !== 'undefined') {
      const userId = BX24.getUserId();
      this.currentUser = {
        ID: userId,
        NAME: 'User'
      };
  ...
  async saveVisit(complete = false) {
    ...
    try {
      ...
      const taskData = { 
        ...
        responsibleId: this.currentUser?.ID || 1, 
        projectId: projectId, 
        auditors: auditors, 
        files: uploadedFiles 
      };
