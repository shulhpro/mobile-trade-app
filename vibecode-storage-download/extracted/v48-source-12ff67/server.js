...
app.get('/api/my-tasks', async (req, res) => {
  try {
    ...
    console.log('Loaded tasks for user', user.id, ':', tasks.length);
...
// Add comment to task (with optional files)
app.post('/api/tasks/:id/comment', upload.array('files', 5), async (req, res) => {
  try {
    ...
    if (!text.trim() && (!req.files || req.files.length === 0)) {
      return res.status(400).json({ error: 'Comment text or files required' });
    }
    ...
    if (req.files && req.files.length > 0) {
      for (const file of req.files) {
        try {
          ...
          const uploadResponse = await fetch(VIBECODE_API + '/storage/objects/upload', {
            method: 'POST',
            headers: {
              'X-Api-Key': API_KEY,
          ...
          if (uploadResponse.ok) {
            ...
            fileUrls.push(uploadData.url || uploadData.data?.url);
          }
        } catch (e) {
    ...
    if (fileUrls.length > 0) {
      if (message) message += '\n\n';
      message += 'Файлы:\n' + fileUrls.map(url => `[URL=${url}]${url}[/URL]`).join('\n');
    }
    
    // Use direct REST endpoint for comments
    ...
    res.json({ success: true, message: 'Comment added', commentId: data.data?.id });
  } catch (error) {
