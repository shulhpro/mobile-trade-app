...
function selectCompany(companyId) {
  ...
  document.getElementById('photoCompanyName').textContent = companyName;
  document.getElementById('noteCompanyName').textContent = companyName;
  document.getElementById('orderCompanyName').textContent = companyName;
...
document.getElementById('visitForm')?.addEventListener('submit', async (e) => {
  ...
  try {
    const response = await fetch('/api/activities', {
      ...
      body: JSON.stringify({
        companyId: currentCompany.id || currentCompany.ID,
        type: 'visit',
...
document.getElementById('photoForm')?.addEventListener('submit', async (e) => {
  ...
  try {
    const response = await fetch('/api/activities/with-photo', {
      method: 'POST',
      body: formData
    });
